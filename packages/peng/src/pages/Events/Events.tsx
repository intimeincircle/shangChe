...
const Events: FunctionComponent = () => {
    ...
    useEffectAsync(async () => {
        ...
        const events = await getEventsByStock(code)

        setEvents(events.records ?? [])

        const chaodie = await getChaodieByStock(code)
        console.log('v', events);
        const history = await getHistoryByStock(code)
        console.log('history head', dayjs(history?.records?.[0]?.date))
        const recordSize = history?.records?.length
        console.log('history last', dayjs(history?.records?.[recordSize-1]?.date))
        const Data = history.records.map(r => ({...r, date: dayjs(r.date)})).sort((leftItem, b) => leftItem.date.isAfter(b.date) ? 1: -1).map(r => ({
        ...
        console.log('Da', Data11dd)
        setDaList(Data11dd);
        const Data1 = chaodie.records.map(r => ({...r, date: dayjs(r.date)})).sort((iii1, b) => iii1.date.isAfter(b.date) ? 1: -1).map(r => ({
            x: dayjs(r.date).format('YYYY-MM-DD'),
            y: Number(r.chaodie)
        }))
        console.log('D', Data)
