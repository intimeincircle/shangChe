...
import {
    ...
    IChatService,
    IStorage, MessageContentType, MessageDirection, SendMessageServiceParams,
    SendTypingServiceParams,
    UpdateState, UserTypingEvent
...
export class E12xampleChatService implements IChatService {
    storage?: IStorage;
    ...
    constructor(storage: IStorage, update: UpdateState) {
        ...
        window.addEventListener("chat-protocol", (evt: Event) => {
            ...
            if (type === "message") {
                ...
                console.log('eventHandlers = ', this.eventHandlers.onMessage);
                console.log('detail.sender = ', detail.sender);
                if (this.eventHandlers.onMessage && detail.sender !== this) {
                    console.log('onMessage this.eventHandlers.onMessage', this.eventHandlers.onMessage)
                    ...
                    const ad  = new MessageEvent({ message, conversationId })
                    console.log('addddddddddddddddd', ad);
                    this.eventHandlers.onMessage(ad);
                }
            } else if (type === "typing") {
                ...
                if (this.eventHandlers.onUserTyping && sender !== this) {
                    ...
                    // 2. Debounce
                    // 3. Re-render
                    const evnt = new UserTypingEvent({
                        userId,
                        isTyping,
                        conversationId,
                        content,
                    ...
                    this.eventHandlers.onUserTyping(evnt);
                }
            }
        });
    }
